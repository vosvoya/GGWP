--[[
  ...
  One-click workflow:
    ...
    2. GGWP renders the group's post-fader MAIN 1/2 output to a MONO stem.
       Auxiliary track channels (3/4, 5/6, etc.) remain available to FX/
       sidechains during processing, but are not written as extra PRINT channels.
  ...
  v1.0.2 render fix:
    - Replaced the multichannel parent-send stem render (Action 42589)
      with REAPER's native mono post-fader stem render (Action 40537).
    - GGWP PRINT is now always a one-channel mono proxy instead of inheriting
      the source track's multichannel width.
    - This prevents auxiliary/sidechain channels 3+ from becoming extra
      channels in the rendered visual proxy.
    - The rendered media is verified to be exactly 1 channel before GGWP
      accepts it as a valid PRINT.
...
then
  reaper.ShowMessageBox(
    "GGWP v1.0.2 requires REAPER 7.30 or newer.\n\nDetected: " .. APP_VERSION,
    "GGWP v1.0.2",
    0
  )
  return
...
-- IMPORTANT:
-- 40537 = Track: Render tracks to mono post-fader stem tracks
--         (and mute originals)
--
-- GGWP deliberately uses MONO here. The source group may internally have
-- 4/6/8+ channels for sidechains and auxiliary routing; those channels must
-- remain present while FX process the group, but the visual PRINT itself must
-- represent only the group's main 1/2 program output summed to one channel.
local ACTION_RENDER_GROUP_POST_FADER_MONO = 40537
...
local function error_box(text)
  reaper.ShowMessageBox(text, "GGWP v1.0.2", 0)
end
...
if folder_depth ~= 1 then
  error_box(
    ...
    "GGWP v1.0.2 expects a summing folder/group track."
  )
  return
end
...
local function setup_fail(message, new_tracks_to_delete)
  ...
  finish_setup_undo("GGWP v1.0.2 - Setup failed")
...
if automation_item_count > 0 then
  setup_fail(
    "GGWP v1.0.2 does not yet support Track Volume Automation Items.\n\n" ..
...
------------------------------------------------------------
-- RENDER CURRENT GROUP OUTPUT — MONO MAIN 1/2 ONLY
--
-- Action 40537 creates a mono post-fader stem. Unlike the old multichannel
-- action, the PRINT no longer inherits the group's track-channel count.
-- Internal channels 3+ can still feed FX/sidechains during processing, but
-- the resulting proxy media is a single mono channel.
------------------------------------------------------------
...
reaper.Main_OnCommand(
  ACTION_RENDER_GROUP_POST_FADER_MONO,
  0
)
...
-- v1.0.2 safety verification:
-- The visual proxy must be exactly one channel. If REAPER ever changes the
-- native render action behavior, fail safely instead of accepting a
-- multichannel PRINT and silently showing the wrong waveform.
local print_source = reaper.GetMediaItemTake_Source(print_take)
local print_source_channels =
  print_source and reaper.GetMediaSourceNumChannels(print_source) or 0

if print_source_channels ~= 1 then
  setup_fail(
    "GGWP v1.0.2 expected a mono PRINT, but the rendered media has " ..
    tostring(print_source_channels) .. " channel(s).\n\n" ..
    "The PRINT was rejected to avoid using auxiliary/sidechain channels."
  )
  return
end
...
finish_setup_undo("GGWP v1.0.2 - Create Group Gain Wave Proxy")
...
local function loop()
  ...
  -- Safety behavior: do not keep a hidden live engine running after the user
